...
func DialWithSplitRetry(ctx context.Context, dialer *net.Dialer, addr *net.TCPAddr, stats *RetryStats) (DuplexConn, error) {
	log.Debug("SplitRetry(DialWithSplitRetry) - dialing", "addr", addr)
	before := time.Now()
	conn, err := dialer.DialContext(ctx, addr.Network(), addr.String())
	log.Debug("SplitRetry(DialWithSplitRetry) - dialed", "err", err)
	if err != nil {
		return nil, err
	}
	after := time.Now()
...
func (r *retrier) Read(buf []byte) (n int, err error) {
	...
	if !r.retryCompleted() {
		...
		if err != nil {
			...
			log.Debug("SplitRetry(retrier.Read) - direct conn succeeded, no need to split")
		}
...
func (r *retrier) retry(buf []byte) (n int, err error) {
	log.Debug("SplitRetry(retrier.retry) - retrying...")
	defer func() { log.Debug("SplitRetry(retrier.retry) - retried", "n", n, "err", err) }()
