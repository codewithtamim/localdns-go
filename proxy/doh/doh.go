...
func (r *resolver) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	log.Debug("DoH(resolver.dial) - dialing", "addr", addr)
	...
	if confirmed != nil {
		log.Debug("DoH(resolver.dial) - trying confirmed IP", "confirmedIP", confirmed, "addr", addr)
		if conn, err = split.DialWithSplitRetry(ctx, r.dialer, tcpaddr(confirmed), nil); err == nil {
			log.Info("DoH(resolver.dial) - confirmed IP worked", "confirmedIP", confirmed)
			return conn, nil
		}
		log.Debug("DoH(resolver.dial) - confirmed IP failed", "confirmedIP", confirmed, "err", err)
		ips.Disconfirm(confirmed)
	}

	log.Debug("DoH(resolver.dial) - trying all IPs")
	for _, ip := range ips.GetAll() {
		...
		if conn, err = split.DialWithSplitRetry(ctx, r.dialer, tcpaddr(ip), nil); err == nil {
			log.Info("DoH(resolver.dial) - found working IP", "ip", ip)
			return conn, nil
		}
	}
	return nil, err
...
func (r *resolver) sendRequest(id uint16, req *http.Request) (response []byte, hostname string, server *net.TCPAddr, qerr *queryError) {
	...
	defer func() {
		...
		log.Info("DoH(resolver.sendRequest) - done", "id", id, "queryError", qerr)
		if server != nil {
			log.Debug("DoH(resolver.sendRequest) - disconfirming IP", "id", id, "ip", server.IP)
			r.ips.Get(hostname).Disconfirm(server.IP)
		}
		if conn != nil {
			log.Info("DoH(resolver.sendRequest) - closing failing DoH socket", "id", id)
			conn.Close()
		}
	}()
	...
	req.Header.Set("User-Agent", "LocalDNS")
	log.Debug("DoH(resolver.sendRequest) - sending query", "id", id)
	httpResponse, err := r.client.Do(req)
	if err != nil {
		qerr = &queryError{SendFailed, err}
		return
	}
	log.Debug("DoH(resolver.sendRequest) - got response", "id", id)
	...
	log.Debug("DoH(resolver.sendRequest) - response closed", "id", id)
	...
	if httpResponse.StatusCode != http.StatusOK {
		...
		log.Debug("DoH(resolver.sendRequest) - response invalid", "id", id, "req", reqBuf, "resp", respBuf)
...
func (r *resolver) Query(ctx context.Context, q []byte) ([]byte, error) {
	...
	//   4. Java -   GoDnsListener.onResponse
	...
	LogDNSResolverQuery("DoH", r.url, q, response, before, after, err, errIsCancel)
	return response, err
}
...
func forwardQueryAndCheck(r Resolver, q []byte, c io.WriteCloser) {
	if err := forwardQuery(r, q, c); err != nil {
		log.Warn("DoH(forwardQueryAndCheck) - forwarding failed", "err", err)
		c.Close()
	}
}
...
func Accept(r Resolver, c io.ReadWriteCloser) {
	...
	for {
		n, err := c.Read(qlbuf)
		if n == 0 {
			log.Debug("DoH(Accept) - TCP query socket clean shutdown")
			break
		}
		if err != nil {
			log.Warn("DoH(Accept) - failed to read from TCP query socket", "err", err)
			break
		}
		if n < 2 {
			log.Warn("DoH(Accept) - incomplete query length")
			break
		}
		...
		n, err = c.Read(q)
		if err != nil {
			log.Warn("DoH(Accept) - failed to read query", "err", err)
			break
		}
		if n != int(qlen) {
			log.Warn("DoH(Accept) - incomplete query (n < qlen)", "n", n, "qlen", qlen)
			break
		}
...
func Servfail(q []byte) ([]byte, error) {
	defer log.Debug("DoH(SERVFAIL) - response generated")
...
func tryServfail(q []byte) []byte {
	...
	if err != nil {
		log.Warn("DoH(SERVFAIL) - failed to construct response", "err", err)
	}
	return response
}
