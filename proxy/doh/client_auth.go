...
func (ca *clientAuthWrapper) GetClientCertificate(
	...
	if ca.signer == nil {
		log.Warn("Client certificate requested but not supported")
		return &tls.Certificate{}, nil
	}
	cert := ca.signer.GetClientCertificate()
	if cert == nil {
		log.Warn("Unable to fetch client certificate")
		return &tls.Certificate{}, nil
	}
	chain := [][]byte{cert}
	...
	if !isECDSA {
		...
		log.Warn("Only ECDSA client certificates are supported")
		return &tls.Certificate{}, nil
	}
